...
import { ProjectSidebar, type ProjectSidebarTab } from "@/components/sidebar/project-sidebar";
...
export default function ProjectWorkspace() {
  ...
  const { setProjectId, theme, customTheme } = useAppStore();
  const [mobileView, setMobileView] = useState<"chat" | "canvas">("chat");
  const effectiveTheme = customTheme ? "custom" : theme;

  const initialTab: ProjectSidebarTab =
    tabFromUrl === "resources" || tabFromUrl === "pages"
      ? "resources"
      : tabFromUrl === "timeline"
        ? "timeline"
